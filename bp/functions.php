...
function a21_cart_change_prod_count(){
	...
	$_SESSION["products"][$_POST['prod_id']]['count'] = $_POST['count'];
	$out['cur_prod_sum'] = $_SESSION["products"][$prod_id]['count']*$_SESSION["products"][$prod_id]['sum'];
...
function a21_cart_del_prod(){
	...
	if( !empty($prod_id) && $prod_id > 0){
		...
		$cur_sum = $_SESSION['products'][$prod_id]['sum'] * $_SESSION['products'][$prod_id]['count'];
		$_SESSION['total_sum'] -= $cur_sum;
		$_SESSION['total_count'] -= $_SESSION['products'][$prod_id]['count'];
 ...
 //        $headers  = 'MIME-Version: 1.0'."\r\n";
    ...
    if ($products->have_posts()) : 
		...
		while ($products->have_posts()) : $products->the_post(); ?>
			...
			$prod_sum = $_SESSION['products'][$prod_id]['count']*$_SESSION['products'][$prod_id]['sum'];	
			$total_sum += $prod_sum; 	
			// <td ><img class="product_img" src="http://'.$_SERVER['HTTP_HOST'].$img_src.'" alt=""></td>
			$html .= '<tr id="p_id-'.$prod_id.'">
						<td></td>
						<td class="title"><a href="'.get_permalink().'">'.get_the_title().'</a></td>
						<td style="border-left:1px solid #ccc;padding-left:5px;">'.$price.' р.</td>			
						<td style="border-left:1px solid #ccc;padding-left:5px;">'.$_SESSION['products'][$prod_id]['count'].'</td>
			...
			if($post['user_id']){
				...
				$wpdb->insert(
					$wpdb->posts,
					array( 'post_author' => $num_cur_order, 'post_date'=>current_time('mysql'),'post_type' => 'as21_orders','post_parent'=>$prod_id,'menu_order'=>$price,'comment_count'=> $_SESSION['products'][$prod_id]['count'] ),
					array( '%d','%s', '%s','%d', '%d', '%d' )
				);
			}
		 endwhile; 
...
// exit;
	...
	if( !empty($prod_id) && $prod_id > 0){
		...
		$_SESSION["products"][$prod_id]['count'] = 1;
		$_SESSION["products"][$prod_id]['sum'] = $price;
    ...
    if ($products->have_posts()) : 
		...
		while ($products->have_posts()) : $products->the_post(); ?>
			...
			$html .= '<tr id="p_id-'.$prod_id.'">
						...
						<td>'.$price.' р.</td>			
						<td>
						<input class="prod_count" data-prod-id="'.$prod_id.'" style="width: 20px; text-align: center;" type="text" value="'.$_SESSION['products'][$prod_id]['count'].'"/>
						</td>
						<td class="prod_sum">'.$prod_sum.' р.</td>
						<td><a href="#" class="a21_cart_del_prod" data-prod-id="'.$prod_id.'"><img src="'.get_template_directory_uri().'/images/a21/cart_prod_delete.png"/></a>
						</td>
					</tr>';
