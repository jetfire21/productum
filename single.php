...
if (is_category($products) || in_category($products)) {
    ...
    <?php if (post_custom('Image2')) { ?>
    <a href="<?php echo get_option('home'); ?>/<?php echo post_custom('Image2');?>" rel="shadowbox-as21-test[<?php the_ID(); ?>]">
...
<div class="price_stock">
	...
	<div class="h_2">Наличие: <?php if (post_custom('Available') == 'да') {?>
		...
		<!-- &nbsp;<font style="color:green">Есть</font> -->
		&nbsp;<font>Есть</font>
		<?php } else {?><font style="color:red"><?php echo post_custom('Available'); ?></font><?php }?></div><?php }?>
		<?php if (post_custom('Price')) {?><div class="h_1">Цена: <span><?php echo post_custom('Price'); ?></span> руб</div><?php }?>
...
$values = rwmb_meta('as21_prod_options');
// alex_debug(0,1,'',$values);
// print_r($values);
//    echo '<select name="as21_prod_options" class="as21_prod_options">
//    <option selected="true" disabled="disabled">Выберите вариант...</option>';
//     foreach ( $values as $value ) {
//         // echo $value['title'].' - '. $value['price']."<br>";
//         echo '<option value="'.$value['price'].'">'.$value['title'].'</option>';

// }
//    echo '</select>';
if (!empty($values)):
    echo '<h3 class="head-bg-gray-gradient">Варианты товара</h3>';
